...
class GlitchClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.type == discord.MessageType.reply:
            ...
            "<@1180692348196376626>" in message.content
            or "<@&1183304745151103027>" in message.content
        ):
            needs_response = True
        ...
        if contains_word:
            await message.reply(
                "Your message was caught by my input filter, let Minaro know if you think this is an error"
            )
            return
        ...
        async with message.channel.typing():
            ...
            if matches:
                ...
                self.people_opinions[author_username] = clamp_value(
                    -100, 100, self.people_opinions[author_username] + opinion_change
                )

                write_dict_to_json(
                    self.people_opinions, f"{link_to_memory}/opinions_list.json"
                )
            ...
            contains_word, filtered_word = contains_filtered_word(
    ...
    async def generate_proc_message(
        self, channel, message_count=message_lookback_amount
    ):
        ...
        async for message in channel.history(limit=message_count):
            ...
            if author_member == self.user:
                llm_messages[i] = {"role": "assistant", "content": f"{content}"}
            else:
                message_author = author_member.global_name
                opinion_of_user = (
                    self.people_opinions[message_author]
                    if message_author in self.people_opinions
                    else 0
                )
                llm_messages[i] = {
                    "role": "user",
    ...
    async def send_to_agent(self, proc_message):
        ...
        return_message = self.process_agent_response(
...
def get_day_suffix(day):
    suff_num = day % 10
    day_suff = "th"
    if not (10 <= day <= 20) and (
        suff_num <= 3
    ):  # Why DO we have different names for numbers 10-20??? Whats up with that?
        normal_suffixes = {1: "st", 2: "nd", 3: "rd"}
